...
public final class PhoneNumberFactory {
    ...
    public static PhoneNumber[] createRandomPhoneNumberArray(int phoneNumberCount) {
        ...
        for(int i = 0; i < phoneNums.length; i++)
            phoneNums[i] = createRandomPhoneNumber();
    ...
    public static PhoneNumber createRandomPhoneNumber() {
        int areaCode = RandomNumberFactory.createInteger(200, 800);
    ...
    public static PhoneNumber createPhoneNumberSafely(int areaCode, int centralOfficeCode, int phoneLineCode) {
        ...
        String newNumString = "(" + areaCode + ")-" + centralOfficeCode + "-" + phoneLineCode;
        try {
            newNumber = createPhoneNumber(newNumString);
        } catch(InvalidPhoneNumberFormatException err) {
            logger.log(Level.SEVERE, newNumString + " is not a valid phone number");
            System.out.print("Invalid phone number: " + err);
        }
        return newNumber;
    }
