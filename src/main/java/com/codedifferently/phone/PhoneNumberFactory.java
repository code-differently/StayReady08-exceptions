...
public final class PhoneNumberFactory {
    ...
    public static PhoneNumber createRandomPhoneNumber() {
        int areaCode = createInteger(201, 989);
        int centralOfficeCode = createInteger(201,989);
        int phoneLineCode = createInteger(2222, 9999);
    ...
    public static PhoneNumber createPhoneNumberSafely(int areaCode, int centralOfficeCode, int phoneLineCode) {
        PhoneNumber num;
        String concat = "(" + areaCode + ")" + "-" + centralOfficeCode + "-" + phoneLineCode;
        try{
            num = createPhoneNumber(concat);

        }catch (InvalidPhoneNumberFormatException ex){
            logger.info(concat + "is not a valid phone number");
            return null;
        }
        return num;
    }
